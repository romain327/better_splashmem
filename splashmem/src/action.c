...
void action_do(Player *player, Cell map[MAP_SIZE][MAP_SIZE], char act, sound_effects *sounds) {
    uint32_t on_border;
    switch(act) {
        case ACTION_STILL:
            break;
        ...
        case ACTION_DASH_L:
            ...
            if(check_credits(player, DASH)) {
				...
				Mix_PlayChannel(-1, sounds->dash, 0);
            }
            break;
        case ACTION_DASH_R:
        ...
        case POWERUP_TOURBILOL:
            if(check_credits(player, TOURBILOL_COST)) {
                player->PwrUP_id = POWERUP_TOURBILOL;
                powerup_do(player, map);
            }
            break;
    }
}
...
void dash_left(Player *player, Cell map[MAP_SIZE][MAP_SIZE], uint8_t on_border) {
    if(on_border == DASH) {
        ...
        player->x += MAP_SIZE;
        for(uint8_t i = on_border; i < DASH; i++) {
            player->x -= 1;
            render_player_on_map(map, player);
        }
    }
...
void explode_bomb(Bomb *bomb, Player player, Cell map[MAP_SIZE][MAP_SIZE]) {
    ...
    player.x = bomb->x;
    player.y = bomb->y;
    action_do(&player, map, ACTION_SPLASH, NULL);
    player.x = x;
    player.y = y;
}
...
void powerup_do(Player *player, Cell map[MAP_SIZE][MAP_SIZE]) {
    switch(player->PwrUP_id) {
        case POWERUP_TOURBILOL:
            player->powerup_timer ++;
            if(player->powerup_timer == 6) {
                player->powerup_timer = 0;
            }
            if( (((player->x)+2)<96) && (((player->y)+2<96)) && (((player->x))> 4) && (((player->y))> 4) ) {
            uint32_t x = player->x;
            uint32_t y = player->y;
                switch(player->powerup_timer) {
                case 0:
                    player->x = player->x + 3;
                    player->y = player->y + 3;
                    render_player_on_map(map, player);
            ...
            player->credits = player->credits - TOURBILOL_COST;
            player->x = x;
            player->y = y;
        }
