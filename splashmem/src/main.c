...
int main(int argc, char *argv[]) {
    ...
    sound_effects *sounds = malloc(sizeof(sound_effects));

    if(init(window, renderer, grid, font_color, nb_player, argv, players, map, game_master, pos, sounds) == EXIT_ERROR) {
        return EXIT_ERROR;
    }
    ...
    printf("TTF init\n");
    TTF_Init();
    TTF_Font *font = TTF_OpenFont("/opt/splashmem/assets/fonts/arial.ttf", 24);
    if(font == NULL) {
    ...
    uint8_t *finishing = malloc(sizeof(uint8_t));
    *finishing = 1;
    uint8_t done = 0;
    uint32_t loop = 0;
    char *str[nb_player];
    ...
    while(*finishing) {
        ...
        while(SDL_PollEvent(&e))
            handle(&e, *renderer, *window, finishing);
        ...
        for(uint8_t i = 0; i < nb_player; i++) {
            ...
            if(players[i]->credits > 0) {
                ...
                if(players[i]->en_ultime) {
                    action_do(players[i], map, ACTION_ULTIME, sounds);
                }
                else {
                    action_do(players[i], map, players[i]->get_action(), sounds);
                }
            }
            else {
                ...
                action_do(players[i], map, ACTION_STILL, sounds);
            }
