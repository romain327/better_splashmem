...
void render_bmp(SDL_Renderer *renderer, Player *player, const uint32_t nb_player) {
	...
	printf("inversing image\n");
	printf("%d player->compteur_bmp : %d\n", player->id, player->compteur_bmp);
    ...
    if(player->en_ultime == 1 && player->anim_ultime != 1) {
        printf("rendering ultime\n");
        player_bmp_coord2->h = 400;
        player_bmp_coord2->w = 1100;

        if(player->ultime_timer == 0) {
            player_bmp_coord2->x = 0;
        ...
        printf("rendering ultime textures\n");
        TextureTemp = player->bmp.bmp_catch[1];
        TextureTemp2 = player->bmp.bmp_catch[0];

        printf("texture dims : %d, %d, %d, %d\n", player_bmp_coord2->w, player_bmp_coord2->h, player_bmp_coord2->x, player_bmp_coord2->y);
